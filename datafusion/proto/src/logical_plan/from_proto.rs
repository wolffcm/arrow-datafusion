...
use datafusion_expr::{
    ...
    date_bin_gapfill, date_part, date_trunc, digest, exp,
    expr::{self, Sort, WindowFunction},
    floor, from_unixtime, left, ln, locf, log10, log2,
...
impl From<&protobuf::ScalarFunction> for BuiltinScalarFunction {
    fn from(f: &protobuf::ScalarFunction) -> Self {
        ...
        match f {
            ...
            ScalarFunction::DateBinGapfill => Self::DateBinGapfill,
            ScalarFunction::Locf => Self::LOCF,
    ...
    match expr_type {
        ...
        ExprType::ScalarFunction(expr) => {
            ...
            match scalar_function {
                ...
                ScalarFunction::DateBinGapfill => Ok(date_bin_gapfill(
                    parse_expr(&args[0], registry)?,
                    parse_expr(&args[1], registry)?,
                    parse_expr(&args[2], registry)?,
                )),
                ScalarFunction::Locf => Ok(locf(parse_expr(&args[0], registry)?)),
